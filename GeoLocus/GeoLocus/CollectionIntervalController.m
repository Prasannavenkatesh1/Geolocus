...
@interface CollectionIntervalController ()

@end

@implementation CollectionIntervalController

@synthesize dataTransTable,pickerValue;
...
- (void)viewDidLoad
{
    ...
    transIntervaArr = [[NSMutableArray alloc]initWithObjects:@"Collection interval", nil];
...
-(void)getDataFromDb
{
    ...
    while ([rs next]) {
        NSLog(@"%@",
              [rs stringForColumn:@"collectioninterval"]);
        
        collectionInterval = [rs stringForColumn:@"collectioninterval"];
    }
    NSLog(@"Text One:%@",collectionInterval);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (![pickerValue length]==0) {
        ...
        valLabel.text = collectionInterval;
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    [db beginTransaction];
//    success = [db executeUpdate:@"UPDATE geoLocus SET collectioninterval = ?", [intervalListArr objectAtIndex:row]];
    success = [db executeUpdate:@"UPDATE geoLocus SET collectioninterval = ?", interval];
    ...
    if (success) {
        ...
        while ([rs next]) {
            ...
            replaceWithUpdatedData = [rs stringForColumn:@"collectioninterval"];
